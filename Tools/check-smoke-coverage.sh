...
# observed against two sets it declares. If a case ID is deleted from those
# sets, every future run still reports a clean pass while proving less. Nothing
# on a board can catch that: the board only ever runs what it is asked to run.
#
