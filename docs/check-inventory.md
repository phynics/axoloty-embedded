...
| `embedded-coatyjs-test` | `Makefile` → `Tests/Support/embedded/embedded-coatyjs-test.sh` | A board interoperates with the pinned CoatyJS reference agent | MOVE TO AXOLOTY-EMBEDDED | **Unmigrated**: board + broker + CoatyJS reference agent + role-config build (note A) | certain |
| `embedded-host-test` | `Makefile` → `Tests/Support/embedded/embedded-host-test.sh` | A board interoperates with the host Axoloty Swift runtime over a broker | MOVE TO AXOLOTY-EMBEDDED | **Unmigrated**: board + broker + role-config build (note A) | certain |
...
| `embedded-interop-test` | `Makefile` aggregator | Runs agent/host/CoatyJS/last-will/broker-restart together | MOVE TO AXOLOTY-EMBEDDED | **Unmigrated** with its constituents (note A) | certain |
...
| `embedded-device-info` | `Makefile` → `Tests/Support/embedded/embedded-device-info.sh` | Chip model, MAC, flash size, toolchain versions, and serial identity are recorded | MOVE TO AXOLOTY-EMBEDDED | **Unmigrated** as a device diagnostic; its manifest writer already landed as `Platforms/esp32c6-idf/tools/write-device-manifest.mjs`, and `flash.sh` records the device in the proof | certain |
